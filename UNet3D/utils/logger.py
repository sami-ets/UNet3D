...
from UNet3D.config.configurations import LoggerConfiguration
...
class Logger(object):
    ...
    def log_images(self, inputs: torch.Tensor, targets: torch.Tensor, predictions: torch.Tensor, step: int) -> None:
        sources = {
    ...
    def log_stats(self, phase, values: dict, step: int) -> None:
        tag_value = {
            f'{phase} Loss': values["loss"],
            f'{phase} Metric': values["metric"],
